...
- (void)setProgress:(CGFloat)progress animated:(BOOL)animated {
    ...
    if (animated && _totalAnimationDuration > CSAnimationInterval) {
        ...
        CGFloat varianceFactor = fabs(_targetProgress - _progress);
...
- (void)sizeToFit {
    ...
    self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, _image.size.width *_scale , _image.size.height *_scale );
}

- (void)layoutSubviews {
    _startImageView.frame = _endImageView.frame = _clippingView.frame = CGRectMake(0.0, 0.0, _image.size.width *_scale , _image.size.height *_scale );
