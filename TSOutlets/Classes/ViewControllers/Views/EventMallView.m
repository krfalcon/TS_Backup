...
- (void)createEventListWithEventEntityArray {
    ...
    for (int i = 0; i < _eventListArray.count; i++) {
        ...
        if ([[df dateFromString:eventEntity.endTime] compare:[NSDate date]] == NSOrderedAscending) {
            MDIncrementalImageView *imageView = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(0 * self.scale, 0 * self.scale, self.frame.size.width, 135 * self.scale)];
            ...
            UIView *timeView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.size.width / 2 - 25 / 2 * self.scale, 135 * self.scale, eventView.frame.size.width / 2 + 25 * self.scale, 25 * self.scale)];
            timeView.layer.cornerRadius = 12.5 * self.scale;
            timeView.backgroundColor = Color(205, 205, 205, 1);
            [eventView addSubview:timeView];
            
            UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(- 25 / 2 * self.scale, 115 * self.scale, eventView.frame.size.width - 35 * self.scale, 50 / 2 * self.scale)];
            ...
            UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.frame.size.width / 2, 135 * self.scale, self.frame.size.width / 2, 25 * self.scale)];
            ...
            UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(5 * self.scale, 115 * self.scale, eventView.frame.size.width - 40 * self.scale, 50 / 2 * self.scale)];
            ...
            NSMutableAttributedString *str;
            str = [[NSMutableAttributedString alloc] initWithString:timeString];
            [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:13.0 * self.scale] range:NSMakeRange(0, eventEntity.beginTime.length)];
            ...
            [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:13.0 * self.scale] range:NSMakeRange(eventEntity.beginTime.length + 1, eventEntity.beginTime.length)];
