...
- (void)getInfo {
    CarouselScrollView *imageView = [[CarouselScrollView alloc] initWithFrame:CGRectMake(0, self.titleHeight, self.frame.size.width, self.frame.size.width * 270 / 750) andImages:_eventEntity.carouselArray andTheme:@"Red" andRotate:NO andScale:270.f / 750.f];
    ...
    [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:12.0 * self.scale] range:NSMakeRange(0, _eventEntity.beginTime.length)];
    [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:9.0 * self.scale] range:NSMakeRange(_eventEntity.beginTime.length, 1)];
    [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:12.0 * self.scale] range:NSMakeRange(_eventEntity.beginTime.length + 1, _eventEntity.beginTime.length)];
