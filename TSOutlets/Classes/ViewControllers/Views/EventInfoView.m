...
- (void)getInfo {
    ...
    UIView *nameView = [[UIView alloc] initWithFrame:CGRectMake(170 * self.scale, 5 * self.scale, self.frame.size.width - 140 * self.scale, 40 * self.scale)];
    ...
    UILabel *enNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(20 * self.scale, 25 * self.scale, nameView.frame.size.width - 60 * self.scale, 15 * self.scale)];
    ...
    UIButton *telButton = [[UIButton alloc] initWithFrame:CGRectMake(165 * self.scale, 85 * self.scale, 160 * self.scale, 30 * self.scale)];
    ...
    UIButton *locationButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 150 * self.scale, 50 * self.scale, 150 * self.scale, 30 * self.scale)];
    ...
    [locationIcon setFrame:CGRectMake(- 9 * self.scale, 0 * self.scale, 21 * self.scale, 30 * self.scale)];
    ...
    UIView *locationLine = [[UIView alloc] initWithFrame:CGRectMake(0, 30 * self.scale - 1, 150 * self.scale, 1)];
    ...
    UIButton *logoImageButton = [[UIButton alloc] initWithFrame:CGRectMake(5 * self.scale, 5 * self.scale, 160 * self.scale, 110 * self.scale)];
    [logoImageButton setExclusiveTouch:YES];
    [logoImageButton addTarget:self action:@selector(tappedNameButton) forControlEvents:UIControlEventTouchUpInside];
    [upperView addSubview:logoImageButton];
    
    if (_eventEntity.shopEntity.logoImage) {
        UIImageView *logoImage = [[UIImageView alloc] initWithFrame:CGRectMake(5 * self.scale, 5 * self.scale, 160 * self.scale, 110 * self.scale)];
        ...
        MDIncrementalImageView *imageView = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(5 * self.scale, 5 * self.scale, 160 * self.scale, 110 * self.scale)];
    ...
    if (_eventEntity.carouselArray) {
        CarouselScrollView *bigImage = [[CarouselScrollView alloc] initWithFrame:CGRectMake(0, 0, lowerView.frame.size.width, lowerView.frame.size.width * 270.f / 750.f + 30 * self.scale) andImages:_eventEntity.carouselArray andTheme:@"Red" andRotate:YES andScale:270.f / 750.f];
        [lowerView addSubview:bigImage];
    }
    
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(7.5 * self.scale, 180 * self.scale, self.frame.size.width - (7.5 - 17.5) * self.scale, 35 * self.scale)];
    titleView.layer.cornerRadius = 17.5 * self.scale;
    titleView.backgroundColor = ThemeRed;
    [eventScrollView addSubview:titleView];
    
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(17.5 * self.scale, 0, titleView.frame.size.width - 25 * self.scale, 35 * self.scale)];
    ...
    UIView *timeView = [[UIView alloc] initWithFrame:CGRectMake(110 * self.scale, 210 * self.scale, self.frame.size.width - (110 - 12.5) * self.scale, 25 * self.scale)];
    ...
    [timeStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15.0 * self.scale] range:NSMakeRange(0, _eventEntity.beginTime.length)];
    [timeStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:10.0 * self.scale] range:NSMakeRange(_eventEntity.beginTime.length, 1)];
    [timeStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15.0 * self.scale] range:NSMakeRange(_eventEntity.beginTime.length + 1, _eventEntity.beginTime.length)];
    ...
    UILabel *contentLabel = [[UILabel alloc] initWithFrame:CGRectMake(12.5 * self.scale, 260 * self.scale, self.frame.size.width - 25 * self.scale, lowerView.frame.size.height - 290 * self.scale)];
    ...
    [eventScrollView setContentSize:CGSizeMake(eventScrollView.frame.size.width, 260 * self.scale + contentLabel.frame.size.height)];
}
