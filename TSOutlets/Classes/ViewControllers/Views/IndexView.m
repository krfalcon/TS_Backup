...
- (void)initView
{
    ...
    [indexScrollView setContentSize:CGSizeMake(indexScrollView.frame.size.width, (1350 + _offset) * self.scale)];
...
- (void)createCarousel {
    ...
    count = (int)_indexEntity.carouselArray.count;
    
    indexCarousel = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, indexScrollView.frame.size.width, 210 * self.scale)];
    indexCarousel.contentSize = CGSizeMake(self.frame.size.width * (count + 1), 210 * self.scale);
    indexCarousel.pagingEnabled = YES;
    indexCarousel.showsHorizontalScrollIndicator = NO;
    [indexScrollView addSubview:indexCarousel];
    ...
    [indexCarousel addSubview:topView];
    
    if (count > 0) {
        for (int i = 0; i < count; i ++) {
            MDIncrementalImageView *imgView = [[MDIncrementalImageView alloc]init];
            imgView.frame = CGRectMake((i+1) * self.frame.size.width, 0, self.frame.size.width, 210 * self.scale);
            imgView.showLoadingIndicatorWhileLoading = YES;
            [imgView setImageUrl:[NSURL URLWithString:[NSString stringWithFormat:@"http://mgmt.blots.dgshare.cn/%@",[_indexEntity.carouselArray objectAtIndex:i]]]];
            [indexCarousel addSubview:imgView];
        }
        [self addTimer];
    }
    
    /*
    UIImageView *topView2 = [[UIImageView alloc] initWithFrame:CGRectMake(indexCarousel.frame.size.width,0,indexCarousel.frame.size.width,)];
    topView2.image = [UIImage imageNamed:@"IndexTopImage"];
    [indexCarousel addSubview:topView2];*/
}
...
- (void)nextPage {
    ...
    if (indexCarousel.contentOffset.x >= self.frame.size.width * count) {
        page = 0;
    }else{
        page = indexCarousel.contentOffset.x / self.frame.size.width + 1;
    }
    CGFloat offsetX = page * indexCarousel.frame.size.width;
    CGPoint offset = CGPointMake(offsetX, 0);
    [indexCarousel setContentOffset:offset animated:YES];
}
...
- (void)createButtons {
    ...
    [messageImage setImage:[UIImage imageNamed:@"IndexButton_Exchange"]];
...
- (void)createEvents {
    UIView *eventView = [[UIView alloc] initWithFrame:CGRectMake(0, 440 * self.scale, self.frame.size.width, 380 * self.scale)];
    ...
    UIView *crossLine = [[UIView alloc] initWithFrame:CGRectMake(self.frame.size.width / 2, 205 * self.scale, 0.5f, 185 * self.scale)];
    [crossLine setBackgroundColor:Color(83, 201, 255, 1)];
    [eventView addSubview:crossLine];
    
    UIView *bottomLine = [[UIView alloc] initWithFrame:CGRectMake(0 * self.scale, 300 * self.scale, self.frame.size.width, 0.5f)];
    [bottomLine setBackgroundColor:Color(83, 201, 255, 1)];
    [eventView addSubview:bottomLine];
    
    UIView *bottomLine2 = [[UIView alloc] initWithFrame:CGRectMake(0 * self.scale, 390 * self.scale, self.frame.size.width, 0.5f)];
    [bottomLine2 setBackgroundColor:Color(83, 201, 255, 1)];
    [eventView addSubview:bottomLine2];
    
    if (_indexEntity.mallEvent) {
        mallEventButton = [[UIButton alloc] initWithFrame:CGRectMake(0 * self.scale, 2.5 * self.scale, self.frame.size.width, 165 * self.scale)];
        ...
        UILabel *mallEventLabel = [[UILabel alloc] initWithFrame:CGRectMake(15 * self.scale, 35 * self.scale, mallEventButton.frame.size.width - 10 * self.scale,  20 * self.scale)];
        ...
        [mallEventLabel setFont:[UIFont boldSystemFontOfSize:20 * self.scale]];
        [mallEventButton addSubview:mallEventLabel];
        
        MDIncrementalImageView *mallEventImage = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(0, 70 * self.scale, mallEventButton.frame.size.width, mallEventButton.frame.size.width * 270.f / 750.f)];
    ...
    if (_indexEntity.shopEvent) {
        shopEventButton = [[UIButton alloc] initWithFrame:CGRectMake(0 * self.scale, 220 * self.scale, 185 * self.scale, 85 * self.scale)];
        ...
        UILabel *shopEvent1Label = [[UILabel alloc] initWithFrame:CGRectMake(5 * self.scale, -7 * self.scale, 80 * self.scale, shopEventButton.frame.size.height)];
        [shopEvent1Label setBackgroundColor:AbsoluteClear];
        [shopEvent1Label setText:_indexEntity.shopEvent.title];
        [shopEvent1Label setTextColor:Color(43, 125, 146, 1)];
        [shopEvent1Label setTextAlignment:NSTextAlignmentLeft];
        [shopEvent1Label setNumberOfLines:0];
        [shopEvent1Label setFont:[UIFont boldSystemFontOfSize:15 * self.scale]];
        [shopEventButton addSubview:shopEvent1Label];
        
        MDIncrementalImageView *shopEvent1Image = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(90 * self.scale, 0 * self.scale, shopEventButton.frame.size.width - 90 * self.scale, (shopEventButton.frame.size.width - 90 * self.scale) * 220 / 300 )];
        [shopEvent1Image setImageUrl:[NSURL URLWithString:_indexEntity.shopEvent.shopEntity.logoUrl]];
        [shopEventButton addSubview:shopEvent1Image];
    }
    
    if (_indexEntity.shopEventTwo) {
        shopEventTwoButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width / 2, 220 * self.scale, 185 * self.scale, 85 * self.scale)];
        ...
        UILabel *shopEvent2Label = [[UILabel alloc] initWithFrame:CGRectMake(5 * self.scale, -7 * self.scale, 80 * self.scale, shopEventTwoButton.frame.size.height)];
        [shopEvent2Label setBackgroundColor:AbsoluteClear];
        [shopEvent2Label setText:_indexEntity.shopEventTwo.title];
        [shopEvent2Label setTextColor:Color(43, 125, 146, 1)];
        [shopEvent2Label setTextAlignment:NSTextAlignmentLeft];
        [shopEvent2Label setNumberOfLines:0];
        [shopEvent2Label setFont:[UIFont boldSystemFontOfSize:15 * self.scale]];
        [shopEventTwoButton addSubview:shopEvent2Label];
        
        MDIncrementalImageView *shopEvent2Image = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(90 * self.scale, 0 * self.scale, shopEventTwoButton.frame.size.width - 90 * self.scale, (shopEventTwoButton.frame.size.width - 90 * self.scale) * 220 / 300  )];
        [shopEvent2Image setImageUrl:[NSURL URLWithString:_indexEntity.shopEventTwo.shopEntity.logoUrl]];
        [shopEventTwoButton addSubview:shopEvent2Image];
    }
    
    if (_indexEntity.shopEventThree) {
        shopEventThreeButton = [[UIButton alloc] initWithFrame:CGRectMake(0 * self.scale, 310 * self.scale, 185 * self.scale, 85 * self.scale)];
        [shopEventThreeButton setExclusiveTouch:YES];
        [shopEventThreeButton addTarget:self action:@selector(tappedShopEventThree) forControlEvents:UIControlEventTouchUpInside];
        [eventView addSubview:shopEventThreeButton];
        
        UILabel *shopEvent3Label = [[UILabel alloc] initWithFrame:CGRectMake(5 * self.scale, -7 * self.scale, 80 * self.scale, shopEventThreeButton.frame.size.height)];
        [shopEvent3Label setBackgroundColor:AbsoluteClear];
        [shopEvent3Label setText:_indexEntity.shopEventThree.title];
        [shopEvent3Label setTextColor:Color(43, 125, 146, 1)];
        [shopEvent3Label setTextAlignment:NSTextAlignmentLeft];
        [shopEvent3Label setNumberOfLines:0];
        [shopEvent3Label setFont:[UIFont boldSystemFontOfSize:15 * self.scale]];
        [shopEventThreeButton addSubview:shopEvent3Label];
        
        MDIncrementalImageView *shopEvent3Image = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(90 * self.scale, 0 * self.scale, shopEventThreeButton.frame.size.width - 90 * self.scale, (shopEventThreeButton.frame.size.width - 90 * self.scale) * 220 / 300  )];
        [shopEvent3Image setImageUrl:[NSURL URLWithString:_indexEntity.shopEventThree.shopEntity.logoUrl]];
        [shopEventThreeButton addSubview:shopEvent3Image];
    }
    
    if (_indexEntity.shopEventFour) {
        shopEventFourButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width / 2, 310 * self.scale, 185 * self.scale, 85 * self.scale)];
        [shopEventFourButton setExclusiveTouch:YES];
        [shopEventFourButton addTarget:self action:@selector(tappedShopEventFour) forControlEvents:UIControlEventTouchUpInside];
        [eventView addSubview:shopEventFourButton];
        
        UILabel *shopEvent4Label = [[UILabel alloc] initWithFrame:CGRectMake(5 * self.scale, -7 * self.scale, 80 * self.scale, shopEventButton.frame.size.height)];
        [shopEvent4Label setBackgroundColor:AbsoluteClear];
        [shopEvent4Label setText:_indexEntity.shopEventFour.title];
        [shopEvent4Label setTextColor:Color(43, 125, 146, 1)];
        [shopEvent4Label setTextAlignment:NSTextAlignmentLeft];
        [shopEvent4Label setNumberOfLines:0];
        [shopEvent4Label setFont:[UIFont boldSystemFontOfSize:15 * self.scale]];
        [shopEventFourButton addSubview:shopEvent4Label];
        
        MDIncrementalImageView *shopEvent4Image = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(90 * self.scale, 0 * self.scale, shopEventFourButton.frame.size.width - 90 * self.scale, (shopEventFourButton.frame.size.width - 90 * self.scale) * 220 / 300 )];
        [shopEvent4Image setImageUrl:[NSURL URLWithString:_indexEntity.shopEventFour.shopEntity.logoUrl]];
        [shopEventFourButton addSubview:shopEvent4Image];
    }
...
/*
    ...
    UIView *bottomLine = [[UIView alloc] initWithFrame:CGRectMake(0 * self.scale, 172.5 * self.scale, 360 * self.scale, 0.5f)];
    [bottomLine setBackgroundColor:Color(83, 201, 255, 1)];
    [eventView addSubview:bottomLine];
    
    UIView *middleLine = [[UIView alloc] initWithFrame:CGRectMake(170 * self.scale, 95.5 * self.scale, 190 * self.scale, 0.5f)];
    [middleLine setBackgroundColor:Color(83, 201, 255, 1)];
    [eventView addSubview:middleLine];
    
    UIView *rightLine = [[UIView alloc] initWithFrame:CGRectMake(170 * self.scale, 12.5 * self.scale, 0.5f, 160.5 * self.scale)];
    [rightLine setBackgroundColor:Color(83, 201, 255, 1)];
    [eventView addSubview:rightLine];
    
    UIView *crossLine = [[UIView alloc] initWithFrame:CGRectMake(360 * self.scale, 12.5 * self.scale, 0.5f, 160.5 * self.scale)];
    [crossLine setBackgroundColor:Color(83, 201, 255, 1)];
    [eventView addSubview:crossLine];
    
    //mall event
    if (_indexEntity.mallEvent) {
        mallEventButton = [[UIButton alloc] initWithFrame:CGRectMake(0 * self.scale, 2.5 * self.scale, 170 * self.scale, 170 * self.scale)];
        [mallEventButton setExclusiveTouch:YES];
        [mallEventButton addTarget:self action:@selector(tappedMallEvent) forControlEvents:UIControlEventTouchUpInside];
        [eventView addSubview:mallEventButton];
        ...
        MDIncrementalImageView *mallEventImage = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(0, (mallEventButton.frame.size.height - 20 * self.scale) / 2 + 32 * self.scale, mallEventButton.frame.size.width, mallEventButton.frame.size.width * 270.f / 750.f)];
        [mallEventImage setImageUrl:[NSURL URLWithString:_indexEntity.mallEvent.imageUrl]];
        [mallEventButton addSubview:mallEventImage];
    }
    
    if (_indexEntity.shopEvent) {
        shopEventButton = [[UIButton alloc] initWithFrame:CGRectMake(170 * self.scale, 17.5 * self.scale, 190 * self.scale, 75 * self.scale)];
        [shopEventButton setExclusiveTouch:YES];
        [shopEventButton addTarget:self action:@selector(tappedShopEvent) forControlEvents:UIControlEventTouchUpInside];
        [eventView addSubview:shopEventButton];
        ...
        MDIncrementalImageView *mallEventImage = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(90 * self.scale, 0 * self.scale, shopEventButton.frame.size.width - 90 * self.scale, shopEventButton.frame.size.height )];
    ...
    if (_indexEntity.shopEventTwo) {
        shopEventTwoButton = [[UIButton alloc] initWithFrame:CGRectMake(170 * self.scale, 97.5 * self.scale, 190 * self.scale, 75 * self.scale)];
        [shopEventTwoButton setExclusiveTouch:YES];
        [shopEventTwoButton addTarget:self action:@selector(tappedShopEventTwo) forControlEvents:UIControlEventTouchUpInside];
        [eventView addSubview:shopEventTwoButton];
        ...
        MDIncrementalImageView *mallEventImage = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(90 * self.scale, 0 * self.scale, shopEventTwoButton.frame.size.width - 90 * self.scale, shopEventTwoButton.frame.size.height )];
...
- (void)createCategories {
    UIView *categoryView = [[UIView alloc] initWithFrame:CGRectMake(0, (650 + _offset) * self.scale, self.frame.size.width, 120 * self.scale)];
...
- (void)createItems {
    UIView *itemView = [[UIView alloc] initWithFrame:CGRectMake(0, (800 + _offset) * self.scale, self.frame.size.width, 330 * self.scale)];
    ...
    for (int i = 0; i < _indexEntity.itemArray.count; i++) {
        ...
        UIView *itemEntityView = [[UIView alloc] initWithFrame:CGRectMake(155 * i * self.scale, 0, 155 * self.scale, 300 * self.scale)];
        [itemScrollView addSubview:itemEntityView];
        ...
        [itemEntityView addSubview:itemImage];
        ...
        [itemEntityView addSubview:itemName];
        ...
        [itemEntityView addSubview:titleView];
        ...
        [itemEntityView addSubview:leftView];
        ...
        [itemEntityView addSubview:rightView];
        ...
        [itemEntityView addSubview:priceView];
        ...
        [itemEntityView addSubview:saleView];
        ...
        [itemEntityView addSubview:itemButton];
        
        [itemScrollView setContentSize:CGSizeMake(155 * (i + 1) * self.scale, itemScrollView.frame.size.height)];
    }
}

- (void)createAdvertisements {
    UIView *advertisementView = [[UIView alloc] initWithFrame:CGRectMake(0, (1160 + _offset) * self.scale, self.frame.size.width, 180 * self.scale)];
...
- (void)refreshIndexView {
    NSBlockOperation *operation = [NSBlockOperation blockOperationWithBlock:^(){
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            //mall event
                [[indexCarousel subviews] makeObjectsPerformSelector:@selector(removeFromSuperview)];
                [self deleteTimer];
                count = (int)_indexEntity.carouselArray.count;
            ...
            if (_indexEntity.shopEventThree) {
                [[shopEventThreeButton subviews] makeObjectsPerformSelector:@selector(removeFromSuperview)];
            ...
            if (_indexEntity.shopEventFour) {
                [[shopEventFourButton subviews] makeObjectsPerformSelector:@selector(removeFromSuperview)];
            ...
            for (int i = 0; i < _indexEntity.itemArray.count; i++) {
                ...
                UIButton *itemButton = [[UIButton alloc] initWithFrame:itemEntityView.bounds];
                [itemButton setExclusiveTouch:YES];
                [itemButton setTag:i];
                [itemButton addTarget:self action:@selector(tappedItemButton:) forControlEvents:UIControlEventTouchUpInside];
                [itemEntityView addSubview:itemButton];
...
- (void)tappedButton:(UIButton*)button
{
    switch (button.tag) {
        ...
        case 4:
            if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
                [_delegate indexViewDidTappedButton:self withType:ViewControllerTypeSale andParameter:nil];
            }
            break;
            //        case 5:
            //            if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
        ...
        case 7:
             if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
             [_delegate indexViewDidTappedButton:self withType:ViewControllerTypeExchange andParameter:nil];
             }
             break;
        case 8:
            if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
        ...
        case 10:
            if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
                [_delegate indexViewDidTappedButton:self withType:ViewControllerTypeGame andParameter:nil];
            }
            break;
        case 11:
            if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
        ...
        case -1:
            if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
                [_delegate indexViewDidTappedButton:self withType:ViewControllerTypeWifi andParameter:nil];
            }
            break;
        default:
        {
...
- (void)tappedShopEventThree {
    NSDictionary *parameter = [NSDictionary dictionaryWithObject:_indexEntity.shopEventThree forKey:@"entity"];
    if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
        [_delegate indexViewDidTappedButton:self withType:ViewControllerTypeEventInfo andParameter:parameter];
    }
}

- (void)tappedShopEventFour {
    NSDictionary *parameter = [NSDictionary dictionaryWithObject:_indexEntity.shopEventFour forKey:@"entity"];
    if (_delegate && [_delegate respondsToSelector:@selector(indexViewDidTappedButton:withType:andParameter:)]) {
        [_delegate indexViewDidTappedButton:self withType:ViewControllerTypeEventInfo andParameter:parameter];
    }
}
