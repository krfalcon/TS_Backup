...
- (void)createEventList{
    ...
    for (int i = 0; i < _eventListArray.count; i++) {
        ...
        if (entity.imageUrl > 0) {
            UIButton *eventView = [[UIButton alloc] initWithFrame:CGRectMake(10 * self.scale, h * self.scale, self.frame.size.width - 20 * self.scale, 185 * self.scale)];
            ...
            */
            ...
            MDIncrementalImageView *imageView = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(eventView.frame.size.width / 2, 55 * self.scale, eventView.frame.size.width * 0.5f, eventView.frame.size.width * 0.5f * 254 /350 )];
            //imageView.contentMode = UIViewContentModeScaleAspectFit;
            [imageView setShowLoadingIndicatorWhileLoading:YES];
            if ([entity.listImageUrl  isEqual: @"http://app.blots.dgshare.cn/(null)"]) {
                [imageView setImageUrl:[NSURL URLWithString:entity.imageUrl]];
            } else{
                [imageView setImageUrl:[NSURL URLWithString:entity.listImageUrl]];}
