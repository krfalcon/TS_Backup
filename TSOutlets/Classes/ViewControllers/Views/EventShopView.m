...
- (void)initView {
    ...
    searchView = [[SearchTextView alloc] initWithFrame:CGRectMake(160 * self.scale, self.titleHeight, eventScrollView.frame.size.width - 170 * self.scale, 60 * self.scale) andColor:ThemeRed_255_184_194];
...
- (void)tappedEventButton:(UIButton *)button {
    ...
    EventEntity *ety;
    if ([[[button superview] superview] isEqual:eventScrollView]) {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(eventShopView:didTapEventButtonWithEventEntity:)]) {
        [_delegate eventShopView:self didTapEventButtonWithEventEntity:ety];
    }
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if ([scrollView isEqual:eventScrollView]) {
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(eventShopView:didSearch:)]) {
        [_delegate eventShopView:self didSearch:searchText];
    }
}

- (void)searchTextView:(SearchTextView *)searchTextView didStartTexting:(NSString *)searchText {
    if (_delegate && [_delegate respondsToSelector:@selector(eventShopView:didStartTexting:)]) {
        [_delegate eventShopView:self didStartTexting:searchText];
    }
}

- (void)searchTextView:(SearchTextView *)searchTextView didEndTexting:(NSString *)searchText {
    if (_delegate && [_delegate respondsToSelector:@selector(eventShopView:didEndTexting:)]) {
        [_delegate eventShopView:self didEndTexting:searchText];
    }
}
