...
- (void)getInfo:(MemberEntity *)memberEntity {
    ...
    UIButton *memberPortraitButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 130 * self.scale, 130 * self.scale)];
    [memberPortraitButton addTarget:self action:@selector(tappedImageButton) forControlEvents:UIControlEventTouchUpInside];
    [memberPortraitButton setCenter:CGPointMake(self.center.x, 95 * self.scale)];
    [memberPortraitButton setExclusiveTouch:YES];
    [memberScrollView addSubview:memberPortraitButton];
    ...
    //memberPortrait.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:_memberEntity.image]]];
    ...
    UILabel *memberPhone = [[UILabel alloc] initWithFrame:CGRectMake(60 * self.scale, 0, 140 * self.scale, 35 * self.scale)];
