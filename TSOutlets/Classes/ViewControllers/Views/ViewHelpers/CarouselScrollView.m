...
- (id)initWithFrame:(CGRect)frame andImages:(NSArray *)array andTheme:(NSString *)theme andRotate:(BOOL)rotate andScale:(float)s{
    ...
    if (self) {
        ...
        if (count == 1) {
            pageView = [[UIView alloc] initWithFrame:CGRectMake((self.frame.size.width - 18.f * scale + 1 * scale) / 2.f, self.frame.size.height - 19 * scale, 20.f * scale - 3 * scale, 20 * scale)];
            [self addSubview:pageView];
            
            MDIncrementalImageView *imgView2 = [[MDIncrementalImageView alloc]init];
            imgView2.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.width * s );
        ...
        if (count > 1) {
            ...
            for (int i = 0; i < count; i ++) {
                ...
                imgView.frame = CGRectMake(i * self.frame.size.width, 0, self.frame.size.width, self.frame.size.width * s);
           ...
           if (rotate) {
            ...
            MDIncrementalImageView *imgView2 = [[MDIncrementalImageView alloc]init];
            imgView2.frame = CGRectMake(count * self.frame.size.width, 0, self.frame.size.width, self.frame.size.width * s);
            imgView2.showLoadingIndicatorWhileLoading = YES;
            [imgView2 setImageUrl:[NSURL URLWithString:[array objectAtIndex:0]]];
            [carouselScroll addSubview:imgView2];
            if (rotate) {
            ...
            carouselScroll.contentSize = CGSizeMake(self.frame.size.width, self.frame.size.width * s);
            MDIncrementalImageView *imgView = [[MDIncrementalImageView alloc]init];
            imgView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.width * s);
            imgView.showLoadingIndicatorWhileLoading = YES;
