...
#import "ServiceTrafficView.h"

@implementation ServiceTrafficView
...
//    [scrollView addSubview:contentLabel];
    ...
    MDIncrementalImageView *contentImage = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(0 , 0 , self.frame.size.width, self.frame.size.width * 2314 / 700)];
    [contentImage setImageUrl:[NSURL URLWithString:trafficUrl]];
    [contentImage setIsBlack:NO];
    [contentImage setShowLoadingIndicatorWhileLoading:YES];
    //[contentImage setImage:[UIImage imageNamed:@"ServiceTraffic"]];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if (_delegate && [_delegate respondsToSelector:@selector(serviceTrafficView:didStartDragScrollView:)]) {
        [_delegate serviceTrafficView:self didStartDragScrollView:scrollView ];
    }
}
