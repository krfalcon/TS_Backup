...
- (void)refresh {
    if (_delegate && [_delegate respondsToSelector:@selector(videoMallViewDidRefresh)]) {
        [_delegate videoMallViewDidRefresh];
    }
}
...
- (void)createVideoListWithVideoEntityArray {
    ...
    for (VideoEntity* ety in _videoListArray) {
        ...
        MDIncrementalImageView *imageView = [[MDIncrementalImageView alloc] initWithFrame:CGRectMake(7.5 * self.scale, 12.5 * self.scale, videoView.frame.size.width - 15 * self.scale, (videoView.frame.size.width - 15 * self.scale) * 410 / 710 )];
        ...
        UIView *timeView = [[UIView alloc] initWithFrame:CGRectMake(videoView.frame.size.width - 80 * self.scale, (videoView.frame.size.width - 15 * self.scale) * 410 / 710 , 87.5 * self.scale, 15 * self.scale)];
...
- (void)tappedVideoButton:(UIButton *)button {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(videoMallView:didPlayerVideo:)]) {
        [_delegate videoMallView:self didPlayerVideo:videoEntity.videoUrl ];
    }
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if (_delegate && [_delegate respondsToSelector:@selector(videoMallView:didStartDragScrollView:)]) {
        [_delegate videoMallView:self didStartDragScrollView:scrollView ];
    }
}
