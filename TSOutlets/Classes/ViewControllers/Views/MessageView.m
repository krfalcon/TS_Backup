...
- (void)initMessageView:(NSArray *)array {
    ...
    if (_ifLatest) {
        ...
        UIButton *updateButton = [[UIButton alloc] initWithFrame:CGRectMake(10 * self.scale, 10 * self.scale, selfWidth - 20 * self.scale, 60 * self.scale)];
        updateButton.layer.cornerRadius = 15.f * self.scale;
        updateButton.layer.borderColor = ThemeRed.CGColor;
        updateButton.layer.borderWidth = 1.5f;
        [updateButton setExclusiveTouch:YES];
        [updateButton setClipsToBounds:YES];
        [messageScrollView addSubview:updateButton];
        
        UIImage* updateImage = [UIImage imageNamed:@"MessageSystem"];
        UIImageView *updateIcon = [[UIImageView alloc]initWithFrame:CGRectMake(10 * self.scale, 10 * self.scale, 40 * self.scale, 40 * self.scale)];
        [updateIcon setImage:updateImage];
        [updateButton addSubview:updateIcon];
        
        UILabel *updateTitle = [[UILabel alloc] initWithFrame:CGRectMake(60 * self.scale, 0, self.frame.size.width - 110 * self.scale, 30 * self.scale)];
        [updateTitle setFont:[UIFont systemFontOfSize:18 * self.scale]];
        [updateTitle setText:@"系统消息"];
        [updateTitle setTextColor:Color(65, 65, 65, 1)];
        [updateButton addSubview:updateTitle];
    ...
    for (MessageEntity *ety in array) {
        ...
        if ([ety.type intValue] == 0) {
            ...
            [contentLabel setText:[ety.point isEqualToString:@"<null>"] ? @" " : ety.point];
