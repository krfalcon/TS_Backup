...
- (void)initFirstViewWithParameter:(NSDictionary *)parameter
{
    ...
    [[SingleCase singleCase].coredataOperationQueue addOperationWithBlock:^{
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
            [serviceCustomView getServiceCustomWithString:[ArticleCoreDataHelper getServiceQA]];
        }];
    }];
...
- (void)wheelViewTappedButton2 {
    if (!serviceTrafficView) {
        ...
        [[SingleCase singleCase].coredataOperationQueue addOperationWithBlock:^{
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                [serviceTrafficView getServiceTrafficWithString:[ArticleCoreDataHelper getServiceTraffic]];
            }];
        }];
    }
...
- (void)wheelViewTappedButton3 {
    if (!serviceInfoView) {
        ...
        [[SingleCase singleCase].coredataOperationQueue addOperationWithBlock:^{
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                [serviceInfoView getServiceInfoWithString:[ArticleCoreDataHelper getServiceInfo]];
            }];
        }];
    }
