...
+ (BOOL)setIfLatestVersion:(NSString *)ifLatestVersion {
    if (!ifLatestVersion || [ifLatestVersion isEqualToString:@"(null)"]) { return NO; }
    ...
    if (objects == nil || [objects count] == 0) {
        entity = [NSEntityDescription insertNewObjectForEntityForName:@"Config" inManagedObjectContext:context];
        [entity setValue:ifLatestVersion forKey:@"ifLatestVersion"];
    } else {
        [[objects lastObject] setValue:ifLatestVersion forKey:@"ifLatestVersion"];
    }
...
+ (BOOL)checkIfLatestVersion {
    ...
    if (objects == nil || [objects count] == 0) {
        ...
        if ([[[objects lastObject] valueForKey:@"ifLatestVersion"] isEqualToString:@"1"]) {
            return YES;
        } else {
            return NO;
        }
