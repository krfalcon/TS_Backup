...
+ (ItemEntity *)article2ItemEntity:(NSManagedObject *)article {
    ...
    if ([article valueForKey:@"customField"]) {
        ...
        itemEntity.listImageUrl = [NSString stringWithFormat:@"%@%@", APIAddr, [[res valueForKey:@"ListPic"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
        itemEntity.rate = [NSString stringWithFormat:@"%f", [itemEntity.sale floatValue] / [itemEntity.price floatValue]];
        itemEntity.endTime = [[res valueForKey:@"EndTime"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    }
